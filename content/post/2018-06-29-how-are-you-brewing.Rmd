...
I like to try things out on new data as I learn, so where better to flex my tidyverse muscles than on some data on homebrewing, a subject close to my heart. I like to brew all kinds of things, from the pale and hoppy to the strong, dark and interesting. But what is everyone else brewing? The kaggle [Brewer's Friend](https://www.kaggle.com/jtrofe/beer-recipes/data) dataset has the answers. It contains details of over 70,000 homebrew recipes shared by people who use the site. I use Brewer's Friend myself - I don't actually share my recipes so none of mine are in this dataset, but their [recipe calculator](https://www.brewersfriend.com/homebrew/recipe/calculator) is a useful tool. Plus the name always reminds me of *The Inbetweeners*.

![](/post/2018-06-29-how-are-you-brewing_files/friend.jpg){width=350px}
...
The dataset comes from kaggle in CSV format, so let's import it and see what we've got.[^1]

```{r echo = FALSE}
beerfile <- file.path("~", "Rstuff", "miniproj","recipeData.csv")
```


```{r}
beer <- read_csv(beerfile, col_types = cols(), locale = locale(encoding = "UTF-8"))
names(beer)
```
...
[^1]: I have to confess to cheating ever so slightly here. A few of the beer style names had foreign characters (umlauts etc.) in them, so I edited these names in the CSV because I still haven't figured out encoding in R!
